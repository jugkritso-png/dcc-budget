...
import { Bell, Check, Trash2, ExternalLink, Calendar, Info, CheckCircle2, AlertTriangle, AlertCircle } from 'lucide-react';
...
export function NotificationDropdown() {
    const { notifications, markNotificationAsRead, markAllNotificationsAsRead, deleteNotification } = useBudget();
    ...
    const getIcon = (type?: string) => {
        switch (type) {
            ...
            case 'primary': return <Info className="text-primary-600" size={18} />;
                ...
                <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                    ...
                    <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                        <h3 className="font-extrabold text-gray-900 flex items-center gap-2">
                            การแจ้งเตือน
                            {unreadCount > 0 && <span className="px-2 py-0.5 bg-primary-100 text-primary-700 text-[11px] rounded-full">{unreadCount} ใหม่</span>}
                        </h3>
                        {unreadCount > 0 && (
                            <button
                    ...
                    <div className="max-h-[70vh] overflow-y-auto">
                        {notifications.length > 0 ? (
                                ...
                                {notifications.map((n) => (
                                    ...
                                    >
                                        <div className="flex gap-4">
                                            <div className={`mt-0.5 flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center ${!n.isRead ? 'bg-white shadow-sm' : 'bg-gray-100'}`}>
                                                {getIcon(n.type)}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start gap-2">
                                                    <p className={`text-[14px] leading-tight mb-1 ${!n.isRead ? 'font-bold text-gray-900' : 'font-medium text-gray-700'}`}>
                                                        {n.title}
                                                    </p>
                            ...
                            <div className="flex flex-col items-center justify-center py-16 px-10 text-center">
                                ...
                                <h4 className="font-bold text-gray-900 mb-1">ไม่มีการแจ้งเตือน</h4>
