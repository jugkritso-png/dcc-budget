...
const Layout: React.FC<LayoutProps> = ({
  ...
  const notifications = [
    {
      ...
      message: "คำขอ #REQ-2024-001 ได้รับการอนุมัติแล้ว",
      time: "5 นาทีที่แล้ว",
      unread: true,
      type: "success",
      targetPage: Page.BUDGET,
    },
    {
      id: 2,
      title: "แจ้งเตือนงบประมาณ",
      message: 'หมวด "ค่าเดินทาง" ใกล้เต็มวงเงินแล้ว',
    ...
    },
    {
      id: 3,
      title: "คำขอใหม่",
      message: "มีคำขอเบิกจ่ายใหม่รอการตรวจสอบ",
  ...
  const navItems = [
    ...
    ...(hasPermission("manage_budget") || hasPermission("manage_departments")
      ? [{ id: Page.MANAGEMENT, label: "การจัดการ", icon: Settings }]
      : []),
  ...
  const SidebarContent = ({
    ...
    <>
      <div className="p-6">
        <div
          className={`flex items-center gap-3 mb-8 ${isCollapsed ? "justify-center" : ""}`}
        >
          <div
            className={`bg-gradient-to-br from-white/80 to-white/40 border border-white/60 text-primary-600 rounded-xl shadow-lg shadow-black/5 backdrop-blur-md ${isCollapsed ? "p-2.5" : "p-2"}`}
          >
        ...
        <nav className="space-y-2">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center ${isCollapsed ? "justify-center px-0 py-4" : "px-4 py-3.5"} rounded-2xl transition-all duration-300 group relative overflow-hidden ${
                currentPage === item.id
            ...
            >
              ...
              <item.icon
                className={`${isCollapsed ? "w-6 h-6" : "w-5 h-5"} ${!isCollapsed ? "mr-3" : ""} relative z-10 transition-transform group-hover:scale-110 ${currentPage === item.id ? "text-white" : "text-gray-400 group-hover:text-primary-500"}`}
              />
              {!isCollapsed && (
      ...
      <div className="mt-auto p-6 border-t border-gray-100/50">
        ...
        >
          <LogOut
            className={`${isCollapsed ? "w-6 h-6" : "w-5 h-5"} ${!isCollapsed ? "mr-3" : ""}`}
          />
  ...
  return (
    <div className="flex h-[100dvh] w-screen bg-slate-50/50 overflow-hidden relative selection:bg-primary-200">
      ...
      {/* Desktop Sidebar */}
      <aside
        className={`hidden md:flex flex-col ${isSidebarCollapsed ? "w-20" : "w-72"} bg-white/40 backdrop-blur-2xl border-r border-white/40 shadow-2xl shadow-gray-200/10 z-20 h-screen fixed top-0 left-0 transition-all duration-300`}
      >
      ...
      {/* Main Content Area */}
      <div
        className={`flex-1 flex flex-col h-full overflow-hidden ${isSidebarCollapsed ? "md:ml-20" : "md:ml-72"} transition-all duration-300 relative`}
      >
        {/* Mobile Header (KrungthaiNEXT-style) */}
        <header
          className="md:hidden bg-white sticky top-0 z-40 border-b border-gray-100 px-4 py-2.5 flex items-center justify-between"
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="h-8 w-8 rounded-full bg-primary-600 text-white flex items-center justify-center text-xs font-bold ring-2 ring-primary-100">
              {(() => {
                ...
                return cleanAvatar && cleanAvatar.length <= 3
                  ? cleanAvatar
        ...
        <header className="hidden md:flex h-20 items-center justify-between px-8 py-4 z-40 sticky top-0">
          <div>
            ...
            <p className="text-sm text-gray-500">ยินดีต้อนรับ, {user?.name}</p>
          </div>
                    ...
                    {notifications.map((notif) => (
                      ...
                      >
                        ...
                        <div className="flex gap-3">
                          <div
                            className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${notif.unread ? "bg-primary-500" : "bg-gray-300"}`}
                          ></div>
                          <div>
                            <h4
                              className={`text-sm font-bold mb-0.5 ${notif.unread ? "text-gray-900" : "text-gray-600"}`}
                            >
                              {notif.title}
                            </h4>
