...
export const ApprovalModal: React.FC<ApprovalModalProps> = ({ isOpen, onClose, request }) => {
    const { approveRequest, rejectRequest, user, categories, getApprovalLogs } = useBudget();
        ...
        <div className="flex flex-col w-full gap-3">
            {!canApprove && action === 'view' && (
                <div className="w-full p-3 bg-amber-50 border border-amber-100 rounded-xl text-amber-700 text-xs font-medium mb-1 flex items-center gap-2">
                    <Shield size={14} />
                    ขณะนี้อยู่ในขั้นตอน "{currentStep === 'manager' ? 'หัวหน้าหน่วยงาน' : currentStep === 'finance' ? 'ฝ่ายการเงิน' : 'ผู้อำนวยการ'}" (คุณไม่มีสิทธิ์อนุมัติในขั้นตอนนี้)
                </div>
            )}
        ...
        >
            <div className="space-y-6">
                ...
                <div className="space-y-4 pt-2 border-t border-gray-100">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">เอกสารแนบ (Attachments)</label>
                            ...
                            {request.attachments.map((url, idx) => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col min-w-0">
                                        ...
                                        <span className="text-[10px] text-gray-400 truncate">คลิกเพื่อเปิดดูเป็นหน้าต่างใหม่</span>
                                    </div>
                                </a>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center p-3 bg-gray-50/50 border-b border-gray-100">
                            {[
                                ...
                                <React.Fragment key={step.id}>
                                    ...
                                    {idx < arr.length - 1 && <ArrowRight size={12} className="text-gray-300" />}
                                </React.Fragment>
                            ))}
                        </div>
                                    ...
                                    <div key={log.id} className="flex gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start">
                                                <p className="text-xs font-bold text-gray-900">{log.action === 'approve' ? 'อนุมัติ' : 'ไม่อนุมัติ'} โดย {log.user?.name || 'ผู้ใช้'}</p>
                                                <span className="text-[10px] text-gray-400">{new Date(log.createdAt).toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'short' })}</span>
                                            </div>
                                            <p className="text-[10px] text-gray-500 mt-0.5">ขั้นตอน: {log.stage === 'manager' ? 'หัวหน้าหน่วยงาน' : log.stage === 'finance' ? 'การเงิน' : 'ผู้อำนวยการ'}</p>
                                            {log.comment && <p className="text-xs text-red-600 mt-1 bg-red-50 p-2 rounded-lg border border-red-100">{log.comment}</p>}
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <div className="text-center py-4 text-gray-400 text-xs italic">ยังไม่มีประวัติการพิจารณา</div>
                            )}
                        </div>
                    </div>
                </div>
