...
const CreateRequest: React.FC = () => {
    ...
    const { addRequest, categories, subActivities, user, uploadAttachment } = useBudget();

    const [isUploading, setIsUploading] = useState(false);
    const [selectedFiles, setSelectedFiles] = useState<File[]>([]);

    const [formData, setFormData] = useState<Partial<BudgetRequest> & { expenseItems: ExpenseLineItem[], attachments?: string[] }>({
        project: '',
        documentNumber: '',
        category: '',
        activity: '',
        ...
        <div className="space-y-4 md:space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-2 md:gap-4 px-4 md:px-0">
                <div className="min-w-0 w-full">
                    ...
                    <p className="text-gray-500 text-sm mt-2 md:pl-[52px]">กรอกรายละเอียดเพื่อขออนุมัติงบประมาณสำหรับโครงการหรือกิจกรรม</p>
                </div>
            </div>

            <form onSubmit={handleSubmit} className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full md:max-w-5xl mx-auto">
                <div className="bg-white px-6 py-8 md:p-10 rounded-[24px] shadow-sm border border-gray-100 space-y-8 md:space-y-10">
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 mt-12 border-t border-gray-100 pt-10">
                            <h3 className="text-xl font-extrabold text-gray-900 flex items-center gap-3 tracking-tight">
                                <span className="w-8 h-8 rounded-full bg-primary-50 text-primary-600 flex items-center justify-center text-sm">3</span>
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 mt-12 border-t border-gray-100 pt-10">
                            <h3 className="text-xl font-extrabold text-gray-900 flex items-center gap-3 tracking-tight">
                                <span className="w-8 h-8 rounded-full bg-primary-50 text-primary-600 flex items-center justify-center text-sm">4</span>
                                เอกสารแนบ (ถ้ามี)
                            </h3>
                        </div>
                        <div className="bg-white rounded-[20px] border border-gray-200 shadow-sm p-6 flex flex-col items-center justify-center border-dashed">
                            ...
                            />
                            <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center space-y-2 text-gray-500 hover:text-primary-600 transition-colors">
                                <FileText size={48} className="mb-2 opacity-50" />
                                <span className="text-sm font-semibold">คลิกเพื่อเลือกไฟล์แนบ</span>
                            ...
                            {selectedFiles.length > 0 && (
                                <ul className="mt-4 w-full max-w-md space-y-2">
                                    {selectedFiles.map((f, i) => (
                                        <li key={i} className="flex items-center justify-between text-sm bg-gray-50 px-3 py-2 rounded-lg border border-gray-100">
