...
const BudgetRequestList: React.FC = () => {
    ...
    const getStatusLabel = (status: string, currentStep?: string) => {
        ...
        if (status === 'pending') {
            switch (currentStep) {
                case 'manager': return 'รอหัวหน้าอนุมัติ';
                case 'finance': return 'รอการเงินตรวจสอบ';
                case 'director': return 'รอผู้อำนวยการอนุมัติ';
                default: return 'รอการพิจารณา';
            }
        }
        return status;
    };
            ...
            <div className="space-y-4">
                {filteredRequests.length > 0 ? (
                        ...
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex gap-4 items-start">
                                    <div className="p-3 bg-gray-50 rounded-2xl group-hover:bg-primary-50 transition-colors shrink-0">
                                        <FileText className="text-gray-400 group-hover:text-primary-600" size={24} />
                                    </div>
                                    <div className="min-w-0">
                                        ...
                                        <h3 className="font-bold text-gray-900 group-hover:text-primary-700 transition-colors line-clamp-1">{req.project}</h3>
                                        <p className="text-xs text-gray-500 line-clamp-1">{req.activity}</p>
                                    </div>
                                </div>

                                <div className="flex items-center justify-between md:justify-end gap-8 pl-14 md:pl-0">
                                    <div className="text-right">
                                        <p className="text-lg font-extrabold text-gray-900">฿{req.amount.toLocaleString()}</p>
                                        <div className="flex items-center gap-1.5 justify-end text-[10px] text-gray-400 font-medium">
                                            <Calendar size={12} />
                                            {new Date(req.date).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: '2-digit' })}
                                        </div>
                                    </div>
                            ...
                            {req.status === 'pending' && (
                                <div className="mt-4 pt-4 border-t border-dashed border-gray-100 flex items-center gap-4">
                                    ...
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">ความก้าวหน้าการตรวจสอบ</p>
                                </div>
                            )}
                        </Card>
                    ))
                    ...
                    <Card className="p-12 text-center rounded-[32px] border-dashed border-2 border-gray-100 bg-gray-50/30">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-1">ไม่พบคำขอที่ตรงกับเงื่อนไข</h3>
                        <p className="text-sm text-gray-400">ลองเปลี่ยนเงื่อนไขการค้นหาหรือกลุ่มข้อมูล</p>
                    </Card>
                )}
            </div>
