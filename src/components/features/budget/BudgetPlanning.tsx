    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              กำหนดแผนการใช้จ่ายรายกิจกรรมย่อย (Milestone Timeline)
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-card border border-white/60 ring-1 ring-black/5 p-6 md:p-8 relative">
        ...
        <div className="bg-primary-50/50 border border-primary-100 rounded-2xl p-5 mb-8 flex items-start gap-4">
          ...
          <div>
            <h3 className="text-primary-900 font-bold mb-1 text-sm">
              การใช้งาน Milestone Timeline
            </h3>
            <p className="text-primary-700/80 text-xs leading-relaxed">
              คลิกที่ช่องเดือนเพื่อ "ระบุจำนวนเงิน" ที่วางแผนจะใช้
              แถบสีจะแสดงขึ้นอัตโนมัติเมื่อมียอดเงิน &gt; 0
            </p>
          </div>
        </div>
                ...
                <tr>
                  ...
                  {quarters.map((q) => (
                    <th
                      key={q.id}
                      colSpan={3}
                      className={`${q.color} text-white py-2 px-1 text-center text-xs font-bold border-r border-white/20 last:border-r-0`}
                    >
                      {q.name}
                    </th>
                  ))}
                ...
                <tr className="bg-gray-50/80 text-gray-500 text-xs text-center border-b border-gray-100">
                  ...
                  {quarters.map((q) =>
                    q.months.map((m, i) => (
                      <th
                        key={`${q.id}-${i}`}
                        className="py-3 px-2 border-r border-gray-100 min-w-[90px] group cursor-default hover:bg-gray-100 transition-colors"
                      >
                        <div className="font-bold text-gray-700 group-hover:text-primary-600">
                          {m}
                        </div>
                      </th>
                    )),
              ...
              <tbody className="divide-y divide-gray-50">
                {subActivities.length > 0 ? (
                    ...
                    >
                      <td className="p-4 border-r border-gray-100 bg-white sticky left-0 z-10 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.05)]">
                        <div className="flex flex-col gap-1">
                          <div
                            className="font-bold text-sm text-gray-800 truncate max-w-[200px]"
                            title={sub.name}
                          >
                            {sub.name}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-medium px-2 py-0.5 bg-gray-100 rounded-full text-gray-500 border border-gray-200">
                              Allocated: ฿
                              {sub.allocated > 1000
                                ? (sub.allocated / 1000).toFixed(0) + "k"
                      ...
                      {Array.from({ length: 12 }).map((_, i) => {
                        ...
                        return (
                          ...
                          >
                            {hasAmount && (
                              <div
                                className={`absolute top-2 bottom-2 left-0 right-[-1px] z-10 border-y-2 flex items-center justify-center shadow-sm ${barColor} animate-fade-in`}
                              >
                                <span className="text-[10px] font-bold">
                                  ฿
                                  {plan.amount > 1000
                                    ? (plan.amount / 1000).toFixed(0) + "k"
                                    : plan.amount}
                                </span>
                              </div>
                            )}
                  ...
                  <tr>
                    <td
                      colSpan={13}
                      className="text-center py-16 text-gray-400 bg-white"
                    >
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {editingCell.monthName} - {editingCell.subName}
                </p>
              </div>
              <button
