                    ...
                    <div className="flex-1 overflow-y-auto p-0 bg-gray-50/50">
                        {activeDetailTab === 'requests' ? (
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {req.attachments && req.attachments.length > 0 && (
                                                                <div className="flex items-center gap-1 text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded border border-blue-100 font-bold" title={`${req.attachments.length} attachments`}>
                                                                    <FileText size={10} />
                          ...
                          ${req.status === 'approved' ? 'bg-green-100 text-green-800' :
                                                                req.status === 'pending' || req.status === 'waiting_verification' ? 'bg-yellow-100 text-yellow-800' :
                                                            ...
                                                            {req.status === 'approved' ? 'อนุมัติ' :
                                                                req.status === 'rejected' ? 'ไม่อนุมัติ' :
                                                                    req.status === 'pending' ? (
                                                                        req.currentStep === 'finance' ? 'รอการเงินพิจารณา' :
                                                                            req.currentStep === 'director' ? 'รอผู้อำนวยการพิจารณา' :
                                                                                'รออนุมัติ'
                                                                    ) :
                                                                        req.status === 'waiting_verification' ? 'รอการตรวจสอบ' :
                                                                            req.status === 'completed' ? 'เสร็จสิ้น' : 'อื่น ๆ'}
                                                        </span>
                                                    </td>
                                                </tr>
                                            ))}
