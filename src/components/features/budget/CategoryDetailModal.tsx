...
interface CategoryDetailModalProps {
  ...
  setActiveDetailTab: (
    tab: "requests" | "allocation" | "history" | "expenses",
  ) => void;
  onAdjustBudget: (
    amount: number,
...
const CategoryDetailModal: React.FC<CategoryDetailModalProps> = ({
  ...
  const getCategoryRequests = () => {
    return requests.filter((req) => {
      ...
      return (
        req.category === viewingCategory.name &&
        reqYear === viewingCategory.year
      );
    });
  };

  const categoryRequests = getCategoryRequests();
  const currentSubActivities = subActivities.filter(
    (s) => s.categoryId === viewingCategory.id,
  );
  const totalAllocatedToSub = currentSubActivities.reduce(
  ...
  return (
    <>
      ...
      >
        <div
          className={`bg-white/80 backdrop-blur-2xl border border-white/60 ring-1 ring-black/5 shadow-2xl w-full overflow-hidden animate-fade-in flex flex-col transition-all duration-300 ${
            isFullScreen
              ? "h-full rounded-none"
                            ...
                            >
                              {req.status === "approved"
                                ...
                                : req.status === "rejected"
                                  ? "ไม่อนุมัติ"
                                  : req.status === "pending"
                                    ? req.currentStep === "finance"
                                      ? "รอการเงินพิจารณา"
                                      : req.currentStep === "director"
                                        ? "รอผู้อำนวยการพิจารณา"
                                        : "รออนุมัติ"
                                    : req.status === "waiting_verification"
                                      ? "รอการตรวจสอบ"
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {budgetLogs.map((log) => (
                      <tr key={log.id}>
                        ...
                        <td className="py-3 text-sm text-right font-mono text-gray-600">
...
const SubActivityItem: React.FC<SubActivityItemProps> = ({
  item,
  level = 0,
  onUpdate,
  onDelete,
      ...
      {isAddingChild && (
        <div className="ml-10 mt-2 p-2 bg-blue-50 rounded border border-blue-100 flex gap-2">
          ...
          <input
            ...
            onChange={(e) =>
              setChildForm({ ...childForm, allocated: e.target.value })
            }
          />
          <button
            onClick={() => {
