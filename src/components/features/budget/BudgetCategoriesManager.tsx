...
const BudgetCategoriesManager: React.FC = () => {
  ...
  const [activeDetailTab, setActiveDetailTab] = useState<
    "requests" | "allocation" | "history" | "expenses"
  >("requests");
  ...
  const categoriesWithUsage = categories.map((cat: Category) => {
    ...
    const reserved = (requests as BudgetRequest[])
      .filter(
        (r: BudgetRequest) =>
          r.category === cat.name && r.status === "approved",
      )
      .reduce((sum: number, r: BudgetRequest) => sum + r.amount, 0);

    const pending = (requests as BudgetRequest[])
      .filter(
        (r: BudgetRequest) => r.category === cat.name && r.status === "pending",
      )
      .reduce((sum: number, r: BudgetRequest) => sum + r.amount, 0);
  ...
  const currentYearCategories = categoriesWithUsage.filter(
    (cat: any) => cat.year === selectedYear,
  );

  const filteredCategories = currentYearCategories.filter(
    (cat: any) =>
      cat.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      cat.code.includes(searchQuery),
  );

  const totalAllocated = currentYearCategories.reduce(
    (sum: number, cat: any) => sum + cat.allocated,
    0,
  );
  const totalUsed = currentYearCategories.reduce(
    (sum: number, cat: any) => sum + (cat.used || 0),
    0,
  );
