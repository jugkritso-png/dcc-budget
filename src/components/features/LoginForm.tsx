...
import { User, Lock, ArrowRight, Loader2, AlertCircle, Eye, EyeOff, ShieldCheck } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen w-full flex">
            ...
            <div
                ...
                style={{
                    background: 'linear-gradient(135deg, #0A3A5C 0%, #0A4A80 30%, #0060A8 65%, #1A7AC8 100%)',
                }}
            >
                {/* Subtle radial glow top-right */}
                <div className="absolute top-0 right-0 w-[60%] h-[60%] pointer-events-none"
                    style={{
                        background: 'radial-gradient(ellipse 70% 60% at 80% 10%, rgba(100,180,255,0.10), transparent)',
                    }}
                />
                {/* Subtle radial glow bottom-left */}
                <div className="absolute bottom-0 left-0 w-[70%] h-[50%] pointer-events-none"
                    style={{
                        background: 'radial-gradient(ellipse 80% 60% at 20% 100%, rgba(0,60,120,0.35), transparent)',
                    }}
                />
                ...
                <div className="relative z-10 flex flex-col h-full px-14 py-12 justify-center">
                    ...
                    <div className="mb-10 self-start">
                        <span className="inline-flex items-center px-5 py-2 rounded-full text-[12px] font-semibold"
                            style={{
                        ...
                        <div style={{ marginTop: '-2px' }}>
                            <span
                                ...
                                style={{
                                    fontSize: '60px',
                                    lineHeight: 1.08,
                                    background: 'linear-gradient(90deg, rgba(255,255,255,0.14) 0%, rgba(255,255,255,0.50) 45%, rgba(255,255,255,0.14) 100%)',
                    ...
                    {/* Description */}
                    <p className="mb-10 text-[15px] leading-relaxed max-w-sm"
                        style={{ color: 'rgba(255,255,255,0.55)' }}>
                        ยกระดับการบริหารงบประมาณด้วยความโปร่งใส รวดเร็ว และ
                        แม่นยำ ด้วยระบบ Digital Trust Platform ที่ทันสมัยที่สุด
                    </p>
                    ...
                    <div className="flex gap-4">
                        {[
                            ...
                            <div key={f.label}
                                ...
                                <p className="text-[22px] font-extrabold text-white leading-none">{f.label}</p>
                                <p className="text-[12px] mt-1.5 font-medium"
                                    style={{ color: 'rgba(255,255,255,0.50)' }}>{f.sub}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="w-full max-w-[420px]">
                    ...
                    {error && (
                        <div className="mb-5 flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm"
                            style={{ background: 'var(--accent-red-light)', border: '1px solid #FECACA' }}>
                            <AlertCircle className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--accent-red)' }} />
                            <span className="font-medium" style={{ color: '#B91C1C' }}>{error}</span>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <Lock className="w-[18px] h-[18px]" style={{ color: 'var(--text-tertiary)' }} />
                                </div>
                                <input
                        ...
                        <div className="flex items-center gap-3 py-1">
                            ...
                            <span className="text-[11px] font-medium tracking-widest uppercase"
                                style={{ color: 'var(--text-tertiary)' }}>OR CONTINUE WITH</span>
                            <div className="flex-1 h-px" style={{ background: 'var(--border-default)' }} />
                        </div>
                            ...
                            <GoogleLogin
                                onSuccess={async (credentialResponse) => {
                                    if (credentialResponse.credential) {
                                        ...
                                        } catch { setError('Google Sign-In Error'); }
                                        finally { setIsLoading(false); }
                                    }
                                }}
                                onError={() => setError('Google Sign-In Failed')}
                                shape="pill" theme="outline" size="large"
                                text="signin_with"
                            />
                        </div>
                    ...
                    {/* Copyright */}
                    <p className="mt-10 text-center text-[11px]" style={{ color: 'var(--text-tertiary)' }}>
