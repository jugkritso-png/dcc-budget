...
const ExpenseReport: React.FC = () => {
  ...
  const filteredRequests = useMemo(() => {
    return (requests as BudgetRequest[]).filter((req: BudgetRequest) => {
      ...
      if (viewMode === "verify" && req.status !== "waiting_verification")
        return false;
  ...
  const handleOpenReport = (request: BudgetRequest) => {
    ...
    const items =
      request.expenseItems?.map((item: ExpenseLineItem) => ({
        ...item,
  ...
  const handleExpenseChange = (id: string, value: string) => {
    ...
    setExpenseData((prev: ExpenseLineItem[]) =>
      prev.map((item: ExpenseLineItem) =>
        item.id === id ? { ...item, actualAmount: numValue } : item,
      ),
    );
  };
  ...
  const calculateTotals = () => {
    ...
    const totalActual = expenseData.reduce(
      (sum: number, item: ExpenseLineItem) => sum + (item.actualAmount || 0),
      0,
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload files first
      const uploadedUrls: string[] = [...(selectedRequest.attachments || [])];
      ...
      toast.success(
        "ส่งรายงานผลการตรวจสอบเรียบร้อยแล้ว (Waiting for Verification)",
      );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <AnimatePresence mode="popLayout">
          {(filteredRequests as BudgetRequest[]).map(
            (req: BudgetRequest, index: number) => (
              <motion.div
                key={req.id}
                      ...
                      <div className="flex gap-2">
                        <Button
                          onClick={async () => {
                            if (
                              confirm(
                                "ยืนยันส่งกลับไปตรวจสอบ? (ยอดเงินจะถูกคืนค่ากลับ)",
                              )
                            ) {
      ...
      >
        <div className="space-y-6">
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={(e) =>
                            handleExpenseChange(item.id, e.target.value)
                          }
          ...
          <div className="border-t border-gray-100 pt-6 mt-6">
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-200 border-dashed p-4">
              <input
                type="file"
                multiple
              ...
              >
                ...
                <span className="text-xs font-semibold">
                  คลิกเพื่อเพิ่มไฟล์ใบเสร็จ
                </span>
                <span className="text-[10px]">PDF, PNG, JPG (สูงสุด 10MB)</span>
              </label>
              ...
              {selectedFiles.length > 0 && (
                <div className="mt-4 space-y-2">
                  {selectedFiles.map((file, idx) => (
                    ...
                    >
                      <span className="truncate max-w-[80%] font-medium text-gray-600">
                        {file.name}
                      </span>
                      <button
                        onClick={() =>
        ...
        <div className="space-y-6">
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-gray-500 bg-gray-50 sticky top-0">
                <tr>
                  ...
                  <th className="p-3 rounded-tr-lg text-right">
                    ใช้จ่ายจริง (บาท)
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {selectedRequest?.expenseItems?.map(
                  (item: ExpenseLineItem, index: number) => (
              ...
              <tfoot className="bg-gray-50 font-bold border-t border-gray-200">
                ...
                <tr>
                  <td colSpan={3} className="p-3 text-right text-green-600">
                    เงินคงเหลือคืนงบประมาณ
                  </td>
                  <td className="p-3 text-right text-green-700">
                    ฿
                    {(
                      (selectedRequest?.amount || 0) -
                      (selectedRequest?.actualAmount || 0)
                    ).toLocaleString()}
                  </td>
                </tr>
              </tfoot>
          ...
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col gap-3">
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-500 font-medium tracking-tight uppercase text-[11px]">
                เอกสารแนบทั้งหมด ( All Attachments )
              </span>
              <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-[10px] font-bold">
                {selectedRequest?.attachments?.length || 0} ไฟล์
              </span>
            </div>
            ...
            selectedRequest.attachments.length > 0 ? (
              ...
              <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-lg">
                ...
                <p className="text-xs text-gray-400">ไม่มีเอกสารแนบ</p>
              </div>
            )}
          </div>
        </div>
