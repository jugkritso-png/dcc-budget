...
const SettingsPermissions: React.FC = () => {
  ...
  const togglePermission = (role: string, permissionId: Permission) => {
    ...
    setPermissions((prev) => {
      ...
      if (hasPermission) {
        newRolePermissions = currentRolePermissions.filter(
          (p) => p !== permissionId,
        );
      } else {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h3 className="text-xl font-extrabold text-gray-900 flex items-center gap-2 tracking-tight">
            <Shield className="text-[#0066B3] stroke-[2.5px]" size={24} />
            กำหนดสิทธิ์การใช้งาน (Permissions Matrix)
          </h3>
          <p className="text-gray-500 text-sm mt-1.5 font-medium">
            ตั้งค่าสิทธิ์การเข้าถึงเมนูและการทำงานต่างๆ ของแต่ละ Role
          </p>
        </div>
        {isDirty && (
          <Button
                        ...
                        <button
                          ...
                          className={`w-8 h-8 rounded-[10px] flex items-center justify-center transition-all duration-300 ${
                            isAdmin
                              ...
                              : isEnabled
                                ? "bg-[#00A1E4]/10 text-[#0066B3] shadow-sm hover:scale-110"
      ...
      <div className="bg-gradient-to-br from-blue-50 to-[#F0F4F8] border border-blue-100/50 rounded-[20px] p-5 flex items-start gap-4 shadow-sm">
        <AlertCircle
          className="text-[#0066B3] shrink-0 mt-0.5 stroke-[2px]"
          size={20}
        />
        <div>
          <h4 className="font-bold text-[#005191] text-sm tracking-wide">
            หมายเหตุ
          </h4>
          <p className="text-[#0066B3]/80 text-xs mt-1.5 leading-relaxed font-medium">
            - <strong>Admin</strong> จะมีสิทธิ์ครบถ้วนเสมอ ไม่สามารถแก้ไขได้
            <br />- การเปลี่ยนแปลงสิทธิ์จะมีผลทันทีหลังจากกดบันทึก แต่ User
            ที่ใช้งานอยู่จำเป็นต้อง Refresh หน้าจอเพื่อรับค่าใหม่
          </p>
        </div>
      </div>
    </div>
