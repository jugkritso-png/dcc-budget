        ...
        <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1.5 font-medium">ตั้งค่าสิทธิ์การเข้าถึงเมนูและการทำงานต่างๆ ของแต่ละ Role</p>
                </div>
                {isDirty && (
                    <Button
                        onClick={handleSave}
                                        ...
                                        return (
                                            <td key={`${role.role}-${perm.id}`} className="px-4 py-4 text-center">
                                                <button
                                                    ...
                                                    disabled={isAdmin}
                                                    className={`w-8 h-8 rounded-[10px] flex items-center justify-center transition-all duration-300 ${isAdmin
                                                ...
                                                >
                                                    {isEnabled || isAdmin ? <Check size={18} strokeWidth={3} /> : <X size={18} strokeWidth={2.5} />}
                                                </button>
                                            </td>
                                        );
                                    })}
            ...
            <div className="bg-gradient-to-br from-blue-50 to-[#F0F4F8] border border-blue-100/50 rounded-[20px] p-5 flex items-start gap-4 shadow-sm">
                <AlertCircle className="text-[#0066B3] shrink-0 mt-0.5 stroke-[2px]" size={20} />
                <div>
                    <h4 className="font-bold text-[#005191] text-sm tracking-wide">หมายเหตุ</h4>
