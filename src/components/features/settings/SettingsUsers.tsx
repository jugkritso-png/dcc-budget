    ...
    if (editingUser) {
      ...
      try {
        ...
        if (errorMsg.includes("Username"))
          ...
          toast.error("Email นี้มีผู้ใช้งานแล้ว");
          ...
          {filteredUsers.length === 0 ? (
            <div className="text-center py-32 bg-white/40 backdrop-blur-sm rounded-[32px] border border-dashed border-gray-200 flex flex-col items-center justify-center">
              ...
              <p className="text-gray-500">
                ลองค้นหาด้วยคำอื่น หรือกดปุ่ม "เพิ่มผู้ใช้งาน"
                ...
                {filteredUsers.map((user: any, index: number) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-5 mb-6">
                      <div
                        className={`w-14 h-14 rounded-[16px] flex items-center justify-center text-xl font-bold shadow-sm overflow-hidden shrink-0 ${
                          user.role === "admin"
                            ? "bg-gradient-to-br from-[#00A1E4] to-[#0066B3] text-white"
                        ...
                        {(() => {
                          ...
                          return (
                            <span className="truncate max-w-full px-1">
                              {cleanAvatar && cleanAvatar.length <= 3
                                ? cleanAvatar
                                : user.name.charAt(0)}
                            </span>
                          );
                      ...
                      <div className="min-w-0">
                        <h4 className="text-lg font-bold text-gray-900 group-hover:text-[#0066B3] transition-colors truncate">
                          {user.name}
                        </h4>
                        <p className="text-xs text-gray-500 font-mono bg-gray-50 px-2 py-1 rounded-md border border-gray-100 inline-block mt-1.5 max-w-full truncate">
                          @{user.username}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mt-2 flex-1">
                      <div className="flex items-center gap-3 text-sm text-gray-600 font-medium">
                        <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 shrink-0 group-hover:bg-[#00A1E4]/10 group-hover:text-[#0066B3] transition-colors">
                      ...
                      <div className="flex items-center gap-3 text-sm text-gray-600 font-medium">
                        <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 shrink-0 group-hover:bg-[#00A1E4]/10 group-hover:text-[#0066B3] transition-colors">
                          <Briefcase size={14} className="stroke-[2.5px]" />
                        </div>
          ...
          <div className="space-y-5 py-2">
            ...
            >
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                Password
                {editingUser ? (
                  <span className="ml-2 text-[10px] text-primary-500 font-normal normal-case">
            ...
            >
              ...
              <div className="relative">
                <Select
                  value={userForm.role}
                  onChange={(value: string | null) =>
                    setUserForm({ ...userForm, role: value || "user" })
                  }
                  options={[
