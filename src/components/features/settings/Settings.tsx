   ...
   return (
      <div className="animate-fade-in min-h-[calc(100vh-100px)] space-y-6">
         <div className="mb-8 px-4 md:px-0">
            <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight mb-2">ตั้งค่าระบบ {userRole === 'admin' ? '(Admin)' : ''}</h2>
            <p className="text-gray-500 text-sm md:text-lg mb-6">จัดการข้อมูลพื้นฐานและการตั้งค่าต่างๆ ของระบบ | Role: {userRole || 'none'}</p>
               ...
               <div className="flex flex-wrap gap-2">
                  {menuItems.map((item) => (
                     <button
                        ...
                        className={`flex items-center gap-2.5 px-6 py-3 rounded-[16px] text-[15px] font-bold transition-all duration-300 whitespace-nowrap ${activeTab === item.id
                     ...
                     >
                        <div className={activeTab === item.id ? 'text-white' : 'opacity-70 group-hover:opacity-100 transition-opacity'}>
                           {item.icon}
                        </div>
                        {item.label}
                     </button>
