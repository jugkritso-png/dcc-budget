...
const SettingsSidebar: React.FC<SettingsSidebarProps> = ({ activeTab, setActiveTab, user }) => {
    ...
    return (
        <aside className="w-full md:w-64 shrink-0 animate-in fade-in slide-in-from-left-4 duration-500">
                ...
                {menuItems.map((item) => (
                    <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-200 ${activeTab === item.id
                            ? 'bg-gradient-to-r from-primary-600 to-indigo-600 text-white shadow-lg shadow-indigo-200'
            ...
            <div className="mt-8 p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100 relative overflow-hidden group">
                <div className="absolute -right-4 -top-4 w-20 h-20 bg-indigo-100 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-500"></div>
                <h4 className="font-bold text-indigo-900 mb-2 relative z-10">ดูแลระบบ</h4>
                <p className="text-xs text-indigo-700 mb-4 relative z-10 leading-relaxed">
                    จัดการข้อมูลและการตั้งค่าต่างๆ เพื่อให้ระบบทำงานได้อย่างมีประสิทธิภาพ
                </p>
                <button className="text-xs font-bold text-white bg-indigo-600 px-4 py-2.5 rounded-xl w-full hover:bg-indigo-700 hover:shadow-md transition-all relative z-10">
                    ดูคู่มือการใช้งาน
                </button>
            </div>
        </aside>
